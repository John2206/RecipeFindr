...
const rateLimiter: RateLimiter = {
  ...
  canMakeRequest: function(cost: number = 1): boolean {
    this.refillTokens();
...
interface SearchRecipesRequest extends Request {
  body: {
    ingredients: string[];
    dietary?: string;
    cuisine?: string;
...
interface AskAIRequest extends Request {
  body: {
    prompt?: string;
    ingredients?: string[];
  };
...
router.post('/search-recipes', async (req: SearchRecipesRequest, res: Response) => {
...
// @ts-ignore - Express router type issue, but functionality works correctly
router.post('/ask-ai', async (req: AskAIRequest, res: Response) => {

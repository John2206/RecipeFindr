...
// Interface for ImageNet class data
interface ImageNetClass {
  index: number;
  probability: number;
}

// Interface for food mapping
...
async function preprocessImage(base64Image: string): Promise<tf.Tensor4D> {
    try {
...
async function predict(base64Image: string): Promise<string[]> {
    if (!model) {
    ...
    try {
        ...
        const indices = Array.from(probabilities)
            .map((p: number, i: number): ImageNetClass => ({probability: p, index: i}))
            .sort((a: ImageNetClass, b: ImageNetClass) => b.probability - a.probability)
            .slice(0, 5)
            .map((item: ImageNetClass) => item.index);
        
        // Convert to food ingredients (simplified mapping from ImageNet classes)
        const foodMapping: FoodMapping = {
            'banana': 'Banana',
            'orange': 'Orange',
            'apple': 'Apple',
            'lemon': 'Lemon',
        ...
        if (IMAGENET_CLASSES && IMAGENET_CLASSES.length > 0) {
            detectedIngredients = indices
                ...
                .filter((ingredient: string, index: number, self: string[]) => self.indexOf(ingredient) === index); // Remove duplicates
        } else {
            ...
            detectedIngredients = topIndices.map((index: number) => genericIngredients[index % genericIngredients.length]);
        }
